...
> [!NOTE]
> * **附加数据**，从原始数据中计算出的除了基础数据（eegl_wave，eegr_wave，eeg_alpha_power，eeg_beta_power，eeg_theta_power，eeg_delta_power，eeg_gamma_power）之外的数据。
* 该值目前仅支持设置 eeg
* 值为列表类型
* 列表中的值目前仅支持 channel_power_verbose（双通道脑波能量）；详细返回参数可参考**实时生物数据分析返回值表**
...
```Python
{
    ...
    "kwargs": {
        ...
        "additional_data": {  # 可选参数
            "eeg": 1  # key: eeg、hr，目前只支持eeg；value: ["channel_power_verbose"]
        },
    }
}
```
...
| additional_data | eeg | additional_data 为可选参数；eeg 对应参数值为列表类型，目前只支持 channel_power_verbose
|  | hr | additional_data 为可选参数；eeg 对应参数值为列表类型，目前只支持 channel_power_verbose |
...
| 类别 | 附加类型 | 参数 | 类型 | 取值范围 | 说明 |
| :---: | :---: | :---: | :---: | :---: | :---: |
| eeg |  | eeg_alpha_curve | list[float] | [0, 1]*k, (k≥5) | 脑电α频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  |  | eeg_beta_curve | list[float] | [0, 1]*k, (k≥5) | 脑电β频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  |  | eeg_theta_curve | list[float] | [0, 1]*k, (k≥5) | 脑电θ频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  |  | eeg_delta_curve | list[float] | [0, 1]*k, (k≥5) | 脑电δ频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  |  | eeg_gamma_curve | list[float] | [0, 1]*k, (k≥5) | 脑电γ频段能量变化曲线，横坐标点间间隔为0.6x秒 |
...
|  |  | hr_rec | list[int] | [0, 255]*k, (k≥5) | 心率值全程记录，横坐标点间间隔为0.6x秒 |
|  |  | hrv_avg | float | [0, 255] | 心率变异性平均值 |
|  |  | hrv_rec | list[float] | [0, 255]*k, (k≥5) | 心率变异性全程记录，横坐标点间间隔为0.2秒 |
