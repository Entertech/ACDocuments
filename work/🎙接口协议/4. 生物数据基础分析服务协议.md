...
| 类别 | 参数 | 类型 | 取值范围 | 说明 |
| :---: | :---: | :---: | :---: | :---: |
| eeg | eegl_wave | list[float] | [-500, 500] \* 150 \* upload_cycle（x<=6） | 左通道脑电波形片段，初始阶段返回为空数组，之后返回数组长度为 150 \* upload_cycle | upload_cycle 超过 6 时数组长度最大为 1000
|  | eegr_wave | list[float] | [-500, 500] \* 150 \* upload_cycle（x<=6） | 右通道脑电波形片段，初始阶段返回为空数组，之后返回数组长度为 150 \* upload_cycle | upload_cycle 超过 6 时数组长度最大为 1000
|  | eeg_alpha_power | float | [0, 1] | 脑电α频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 |
|  | eeg_beta_power | float | [0, 1] | 脑电β频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 |
|  | eeg_theta_power | float | [0, 1] | 脑电θ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 |
|  | eeg_delta_power | float | [0, 1] | 脑电δ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 |
|  | eeg_gamma_power | float | [0, 1] | 脑电γ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 |
|  | eegl_alpha_power | float | [0, 1] | 左脑脑电α频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 |
|  | eegl_beta_power | float | [0, 1] | 左脑脑电β频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegl_theta_power | float | [0, 1] | 左脑脑电θ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegl_delta_power | float | [0, 1] | 左脑脑电δ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegl_gamma_power | float | [0, 1] | 左脑脑电γ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegr_alpha_power | float | [0, 1] | 右脑脑电α频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegr_beta_power | float | [0, 1] | 右脑脑电β频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegr_theta_power | float | [0, 1] | 右脑脑电θ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegr_delta_power | float | [0, 1] | 右脑脑电δ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
|  | eegr_gamma_power | float | [0, 1]	右脑脑电γ频段能量占比，初始阶段与信号质量不佳时返回为0，其余情况返回能量占比 | 
...
| hr | hr | int | [0, 255] | 心率值 |
|  | hrv | float | [0, 255] \* 3 \* upload_cycle | 心率变异性，返回数组长度为 3 \* upload_cycle |
...
| 类别 | 参数 | 类型 | 取值范围 | 说明 |
| :---: | :---: | :---: | :---: | :---: |
| eeg | eeg_alpha_curve | list[float] | [0, 1]*k, (k≥20) | 脑电α频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  | eeg_beta_curve | list[float] | [0, 1]*k, (k≥20) | 脑电β频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  | eeg_theta_curve | list[float] | [0, 1]*k, (k≥20) | 脑电θ频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  | eeg_delta_curve | list[float] | [0, 1]*k, (k≥20) | 脑电δ频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  | eeg_gamma_curve | list[float] | [0, 1]*k, (k≥20) | 脑电γ频段能量变化曲线，横坐标点间间隔为0.6x秒 |
|  | eegl_alpha_curve | list[float] | [0, 1]*k, (k≥5) | 左脑脑电α频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegl_beta_curve | list[float] | [0, 1]*k, (k≥5) | 左脑脑电β频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegl_theta_curve | list[float] | [0, 1]*k, (k≥5) | 左脑脑电θ频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegl_delta_curve | list[float] | [0, 1]*k, (k≥5) | 左脑脑电δ频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegl_gamma_curve | list[float] | [0, 1]*k, (k≥5) | 左脑脑电γ频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegr_alpha_curve | list[float] | [0, 1]*k, (k≥5) | 右脑脑电α频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegr_beta_curve | list[float] | [0, 1]*k, (k≥5) | 右脑脑电β频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegr_theta_curve | list[float] | [0, 1]*k, (k≥5) | 右脑脑电θ频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegr_delta_curve | list[float] | [0, 1]*k, (k≥5) | 右脑脑电δ频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
|  | eegr_gamma_curve | list[float] | [0, 1]*k, (k≥5) | 右脑脑电γ频段能量变化曲线，横坐标点间间隔为0.6x秒 | 
